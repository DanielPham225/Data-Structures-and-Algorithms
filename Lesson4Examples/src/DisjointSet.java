...
// this is O(N^3) because this algorithm has 3 loops (each loop is O(n))
// it will go through all elements in loop c then loop b then loop a
// best case A[0] = B[0] = C[0]
//worst case: go through all 3 loops
//average case: O(n^3)
  /** Returns true if there is no element common to all three arrays. */
...
//same but the third loop only runs when a == b
// worst: O(n^3)
  // best: a = b so O(1)
  // average: O(n^2 + t * n)
}
